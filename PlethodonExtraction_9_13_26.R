## Set working directory to "Raw Plethodon Data"

ALLDATA <- NULL

filelist <- list.files()

# Import the calibration file, "PlethodonCalibrationData.csv"
...
for(i in 1:length(filelist)){
  ...
  species <- str_sub(filename, start = 1, end = 2)
  individual <- str_sub(filename, start = 4, end = 6)
  temperature <- str_sub(filename, start = 9, end = 10)
  date <- str_sub(filename, start = 13, end = 21)
...
write.csv(ALLDATA, "PlethodonData.csv", row.names = FALSE)
